...
document.addEventListener('DOMContentLoaded', function ready() {
  const enterHopsEl = document.getElementById('enter-hop-count');
  const dialpadEl = document.getElementById('dialpad');
  const dialpadKeyEls = dialpadEl.querySelectorAll('button');
  const resultsEl = document.getElementById('results');
  const startingKey1El = document.getElementById('starting-key-1');
  const startingKey2El = document.getElementById('starting-key-2');
  const hopCountEl = document.getElementById('hop-count');
  const pathCountEl = document.getElementById('path-count');
  const countTimingEl = document.getElementById('count-timing');
  const acyclicPathsEl = document.getElementById('acyclic-paths');
  const pathsTimingEl = document.getElementById('paths-timing');
  ...
  function onHoverKey(evt) {
    ...
    // a hovered key
    const reachable = el.matches('button:hover')
  ...
  function onClickKey(evt) {
    ...
    if (clickedEl.matches('#dialpad > button')) {
      ...
      if (acyclicPaths.length > 0) {
        ...
        for (const path of acyclicPaths) {
